#!/usr/bin/env python3
"""Carla_Node : Woks as a dummy carla
   Subscribes : Control_Actions
   Calculates : Uses the odomerty and calculate new states based on
   Publishes  : Vizualization Markers, Vehicle States 
"""
import casadi as cd
import rclpy
...
class CarlaHandler(Node):
    def __init__(self, node_name):
        ...
        vehicle_constrinats = VehicleConstraints(1.5, 0.3, -0.3, (pi/6), -(pi/6), inf, -inf, inf, -inf, inf, -inf, inf, -inf, 0.3)
    ...
    def state_publisher_callback(self):
        if (not(self.is_target_reached)):
            ...
            self.waypoints_publisher.publish(self.trajectory_msg)  
