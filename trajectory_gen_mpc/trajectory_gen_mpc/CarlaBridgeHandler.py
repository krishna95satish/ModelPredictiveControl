...
class CarlaBridgeHandler(Node):
    def __init__(self, node_name):
        ...
        self.states.final_state_x = 34.5
        ...
        # Obstacle callbacks
        self.obstacle_pose_subscriber = self.create_subscription(Odometry, "/carla/vehicle_01/odometry", self.obstacle_1_pose_callback, 1000)
        self.obstacle_velocity_subscriber = self.create_subscription(CarlaEgoVehicleStatus, "/carla/vehicle_01/vehicle_status",\
                                                                                  self.obstacle_1_vehicle_status_callback, 1000)
                                                                            
        self.obstacle_pose_subscriber = self.create_subscription(Odometry, "/carla/vehicle_02/odometry", self.obstacle_2_pose_callback, 1000)
        self.obstacle_velocity_subscriber = self.create_subscription(CarlaEgoVehicleStatus, "/carla/vehicle_02/vehicle_status",\
                                                                                  self.obstacle_2_vehicle_status_callback, 1000)                                                                            

        self.obstacle_pose_subscriber = self.create_subscription(Odometry, "/carla/vehicle_03/odometry", self.obstacle_3_pose_callback, 1000)
        self.obstacle_velocity_subscriber = self.create_subscription(CarlaEgoVehicleStatus, "/carla/vehicle_03/vehicle_status",\
                                                                                  self.obstacle_3_vehicle_status_callback, 1000)

        self.obstacle_pose_subscriber = self.create_subscription(Odometry, "/carla/vehicle_04/odometry", self.obstacle_4_pose_callback, 1000)
        self.obstacle_velocity_subscriber = self.create_subscription(CarlaEgoVehicleStatus, "/carla/vehicle_04/vehicle_status",\
                                                                                  self.obstacle_4_vehicle_status_callback, 1000)
    ...
    def obstacle_1_pose_callback(self, obstacle_pose):
        try:
            if not(self.is_target_reached):
                self.states.obstacle_1_state_x =   obstacle_pose.pose.pose.position.x
                self.states.obstacle_1_state_y =   obstacle_pose.pose.pose.position.y
                obstacle_orientation = self.calculate_yaw_from_quaternion(self.obstacle_1_orientaion_quaternion)
                self.states.obstacle_1_velocity_x =  self.obstacle_1_velocity * np.cos(obstacle_orientation)  # V_x = V * cos(theta)
                self.states.obstacle_1_velocity_y =  self.obstacle_1_velocity * np.sin(obstacle_orientation)  # V_y = V * sin(theta)
        except AttributeError:
            print("Please rerun the node, HINT : Callback Sync Failure in Obstacle pose callback")
        except KeyboardInterrupt:
            print("Interrupted")

    def obstacle_1_vehicle_status_callback(self, vehicle_status):
        try:
            self.obstacle_1_velocity = vehicle_status.velocity
            self.obstacle_1_orientaion_quaternion = vehicle_status.orientation
        except AttributeError:
            print("Please rerun the node, HINT : Callback Sync Failure in obstacle_vehicle_status_callback")

    def obstacle_2_pose_callback(self, obstacle_pose):
        try:
            if not(self.is_target_reached):
                self.states.obstacle_2_state_x =   obstacle_pose.pose.pose.position.x
                self.states.obstacle_2_state_y =   obstacle_pose.pose.pose.position.y
                obstacle_orientation = self.calculate_yaw_from_quaternion(self.obstacle_2_orientaion_quaternion)
                self.states.obstacle_2_velocity_x =  self.obstacle_2_velocity * np.cos(obstacle_orientation)  # V_x = V * cos(theta)
                self.states.obstacle_2_velocity_y =  self.obstacle_2_velocity * np.sin(obstacle_orientation)  # V_y = V * sin(theta)
        except AttributeError:
            print("Please rerun the node, HINT : Callback Sync Failure in Obstacle pose callback")
        except KeyboardInterrupt:
            print("Interrupted")

    def obstacle_2_vehicle_status_callback(self, vehicle_status):
        try:
            self.obstacle_2_velocity = vehicle_status.velocity
            self.obstacle_2_orientaion_quaternion = vehicle_status.orientation
        except AttributeError:
            print("Please rerun the node, HINT : Callback Sync Failure in obstacle_vehicle_status_callback")

    def obstacle_3_pose_callback(self, obstacle_pose):
        try:
            if not(self.is_target_reached):
                self.states.obstacle_3_state_x =   obstacle_pose.pose.pose.position.x
                self.states.obstacle_3_state_y =   obstacle_pose.pose.pose.position.y
                obstacle_orientation = self.calculate_yaw_from_quaternion(self.obstacle_3_orientaion_quaternion)
                self.states.obstacle_3_velocity_x =  self.obstacle_3_velocity * np.cos(obstacle_orientation)  # V_x = V * cos(theta)
                self.states.obstacle_3_velocity_y =  self.obstacle_3_velocity * np.sin(obstacle_orientation)  # V_y = V * sin(theta)
        except AttributeError:
            print("Please rerun the node, HINT : Callback Sync Failure in Obstacle pose callback")
        except KeyboardInterrupt:
            print("Interrupted")

    def obstacle_3_vehicle_status_callback(self, vehicle_status):
        try:
            self.obstacle_3_velocity = vehicle_status.velocity
            self.obstacle_3_orientaion_quaternion = vehicle_status.orientation
        except AttributeError:
            print("Please rerun the node, HINT : Callback Sync Failure in obstacle_vehicle_status_callback")

    def obstacle_4_pose_callback(self, obstacle_pose):
        try:
            if not(self.is_target_reached):
                self.states.obstacle_4_state_x =   obstacle_pose.pose.pose.position.x
                self.states.obstacle_4_state_y =   obstacle_pose.pose.pose.position.y
                obstacle_orientation = self.calculate_yaw_from_quaternion(self.obstacle_4_orientaion_quaternion)
                self.states.obstacle_4_velocity_x =  self.obstacle_4_velocity * np.cos(obstacle_orientation)  # V_x = V * cos(theta)
                self.states.obstacle_4_velocity_y =  self.obstacle_4_velocity * np.sin(obstacle_orientation)  # V_y = V * sin(theta)
        except AttributeError:
            print("Please rerun the node, HINT : Callback Sync Failure in Obstacle pose callback")
        except KeyboardInterrupt:
            print("Interrupted")

    def obstacle_4_vehicle_status_callback(self, vehicle_status):
        try:
            self.obstacle_4_velocity = vehicle_status.velocity
            self.obstacle_4_orientaion_quaternion = vehicle_status.orientation
        except AttributeError:
            print("Please rerun the node, HINT : Callback Sync Failure in obstacle_vehicle_status_callback")            
    ...
    def update_map_boundaries(self):
        ...
        if (self.states.initial_state_y < 143.0 ):
            self.states.map_x_max     =  38.5
            self.states.map_x_min     =  34.0
            self.states.map_y_max     =  cd.inf#160.0
            self.states.map_y_min     =  -cd.inf#115.0
            ...
            self.states.map_x_min     =  34.0
            self.states.map_y_max     =  cd.inf#150.0
            self.states.map_y_min     =  -cd.inf#115.0 
            self.states.throttle_max  =  0.5  
            self.states.throttle_min  = -0.5
    ...
    def send_control_actions_to_carla(self, control_actions):
        ...
        if ((cd.norm_inf(self.current_state - self.target_state) > self.error_tolerance) and not(self.is_target_reached)):
            if (control_actions.acceleration > 0.0):
                ...
                self.vehicle_controls.steer = control_actions.steering_angle / -(cd.pi/3)          # Divided by pi/6 to normalize the steering angle values to stay b/w [-1, 1]
                ...
                self.vehicle_controls.throttle = control_actions.acceleration
                self.vehicle_controls.brake = 0.0
                self.vehicle_controls.steer = control_actions.steering_angle / (cd.pi/3)      
                self.vehicle_controls.gear = 1
                self.vehicle_controls.hand_brake = False
                self.vehicle_controls.manual_gear_shift = False
