#!/usr/bin/env python3
"""Dynamic_Obstacle : Creates a obstacle and predicts future states with constant velocity model
   Subscribes : NA
   Calculates : Constant velocity model creation, collision checking, collision avoidance constraints
   Publishes  : NA 
"""
import casadi as cd
...
class DynamicObstacle(Obstacle):
    ...
    def set_collision_avoidance_constraints(self, prediction_horizon, ego_vehicle_diameter, state_X, P_vector):
        ...
        self.position_x = P_vector[0]
        self.position_y = P_vector[1]
        self.velocity_x = P_vector[2]
        self.velocity_y = P_vector[3]
        ...
        for idx in range(prediction_horizon+1):
            ...
            self.move_obstacle(self.velocity_x, self.velocity_y)

        self.lower_bound_constraints[0:(prediction_horizon+1)] = -cd.inf
        self.upper_bound_constraints[0:(prediction_horizon+1)] = 0

    def move_obstacle(self, velocity_x, velocity_y):
        # Constant velocity model : next_state = prev_state + delta_t * velocity
        self.position_x = self.position_x + (self.delta_T * velocity_x)
        self.position_y = self.position_y + (self.delta_T * velocity_y)
