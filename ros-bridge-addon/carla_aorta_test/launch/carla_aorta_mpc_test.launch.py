...
def launch_carla_spawn_object(context, *args, **kwargs):
    carla_spawn_objects_launch = launch.actions.IncludeLaunchDescription(
        ...
        launch_arguments={
            'objects_definition_file': get_package_share_directory('carla_aorta_test') + '/config/objects_mpc_obs_perpendicular.json'
        }.items()
    )
...
def generate_launch_description():
    vehicle_ids = ["vehicle_"+str(i).zfill(2) for i in range(14)]
    temp = [
